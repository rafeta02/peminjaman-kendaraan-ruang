...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pinjamRuang.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pinjam-ruangs.update", [$pinjamRuang->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ruang') ? 'is-invalid' : '' }}" name="ruang_id" id="ruang_id" required>
                    @foreach($ruangs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('ruang_id') ? old('ruang_id') : $pinjamRuang->ruang->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('ruang'))
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('time_start') ? 'is-invalid' : '' }}" type="text" name="time_start" id="time_start" value="{{ old('time_start', $pinjamRuang->time_start) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('time_end') ? 'is-invalid' : '' }}" type="text" name="time_end" id="time_end" value="{{ old('time_end', $pinjamRuang->time_end) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_hp') ? 'is-invalid' : '' }}" type="text" name="no_hp" id="no_hp" value="{{ old('no_hp', $pinjamRuang->no_hp) }}" required>
                @if($errors->has('no_hp'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('penggunaan') ? 'is-invalid' : '' }}" name="penggunaan" id="penggunaan" required>{{ old('penggunaan', $pinjamRuang->penggunaan) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\PinjamRuang::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $pinjamRuang->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('borrowed_by') ? 'is-invalid' : '' }}" name="borrowed_by_id" id="borrowed_by_id">
                    @foreach($borrowed_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('borrowed_by_id') ? old('borrowed_by_id') : $pinjamRuang->borrowed_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('borrowed_by_text') ? 'is-invalid' : '' }}" type="text" name="borrowed_by_text" id="borrowed_by_text" value="{{ old('borrowed_by_text', $pinjamRuang->borrowed_by_text) }}">
